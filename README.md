# OPPL Publications

Publications and related research outputs from the  
**Ocean Physics and Prediction Laboratory (OPPL)**,  
Pukyong National University.

국립부경대학교 **해양역학예측연구실(OPPL)**의  
논문 및 관련 연구 성과를 정리한 저장소입니다.

---

## About | 소개

This repository serves as a curated archive of publications, papers, and related research resources from OPPL.  
It provides a structured entry point to the laboratory’s research outputs, including publication information, article links, citation details, and related datasets or code when available.

이 저장소는 OPPL의 논문과 관련 연구 자원을 체계적으로 정리하기 위한 아카이브입니다.  
논문 정보, 원문 링크, 인용 정보, 그리고 공개 가능한 경우 관련 데이터셋이나 코드까지 연결하는 연구 성과 허브를 목표로 합니다.

---

## Latest Publications | 최근 논문

### 2026
**Assessing vertical coordinate system performance in the Regional Modular Ocean Model 6 configuration for Northwest Pacific**  
Inseong Chang, Young Ho Kim, Young-Gyu Park, Hyunkeun Jin, Gyundo Pak, Andrew C. Ross, Robert Hallberg  
*Geoscientific Model Development*, **19**, 187–216 (2026)  
DOI: `10.5194/gmd-19-187-2026`

### 2024
**Assessment of high-resolution regional ocean reanalysis K-ORA22 for the Northwest Pacific**  
Inseong Chang, Young Ho Kim, Young-Gyu Park, Hyunkeun Jin, Gyundo Pak, Jae-Il Kwon, You-Soon Chang  
*Progress in Oceanography*, **229**, 103359 (2024)  
DOI: `10.1016/j.pocean.2024.103359`

### 2023
**Impact of satellite and regional in-situ profile data assimilation on a high-resolution ocean prediction system in the Northwest Pacific**  
Inseong Chang, Young Ho Kim, Hyunkeun Jin, Young-Gyu Park, Gyundo Pak, You-Soon Chang  
*Frontiers in Marine Science*, **10**, 1085542 (2023)  
DOI: `10.3389/fmars.2023.1085542`

---

## Browse All Publications | 전체 논문 목록

For the complete publication list, see:  
전체 논문 목록은 아래 문서에서 확인할 수 있습니다.

- [docs/publications.md](docs/publications.md)

---

## Research Themes | 연구 주제

- Regional ocean modeling  
  지역 해양 수치모델링
- Ocean reanalysis and prediction  
  해양 재분석 및 예측
- Ocean data assimilation  
  해양 자료동화
- Northwest Pacific and Korean marginal seas  
  북서태평양 및 한국 주변해
- Physical oceanography and ocean circulation  
  물리해양학 및 해양 순환

---

## Repository Structure | 저장소 구성

publications/  
├─ README.md  
├─ docs/  
│  └─ publications.md  
├─ data/  
│  └─ publications.yml  
├─ bib/  
│  └─ oppl-publications.bib  
├─ papers/  
│  ├─ 2023-impact-of-satellite-and-regional-in-situ-profile-data-assimilation/  
│  ├─ 2024-assessment-of-high-resolution-regional-ocean-reanalysis-k-ora22/  
│  └─ 2026-assessing-vertical-coordinate-system-performance-in-mom6/  
└─ assets/

---

## Notes | 참고

- This repository is intended as a **publication catalog**, not only as a PDF archive.
- Related code, datasets, and reproducible research materials may be maintained in separate repositories.
- The structure and contents will be expanded as more publications are added.

- 이 저장소는 단순한 PDF 보관함이 아니라 **논문 카탈로그**를 지향합니다.
- 관련 코드, 데이터, 재현 자료는 별도 저장소에서 관리될 수 있습니다.
- 논문이 추가됨에 따라 구조와 내용은 점진적으로 확장될 예정입니다.

---

## Links | 관련 링크

- **OPPL GitHub Organization**: https://github.com/PKNU-OPPL
- **Laboratory / Department Website**: https://pkuocean.pknu.ac.kr/pkuocean/1480
